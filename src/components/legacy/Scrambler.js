...
class Scrambler extends React.Component {
  ...
  startScrambling(end, start, renderIn = 3000) {
    ...
    for (let i = 0, lastStartFrame = 0, dec = maxFrames; i < longer; i++) {
      ...
      const humanLikeTime =
        lastScrambled.length < end.length
      ...
      const startTransformation =
        this.props.humanLike || this.props.typewriter
          ? humanLikeTime
          : Math.floor(Math.random() * maxFrames * 0.4);
      const transformationDuration = Math.floor(Math.random() * (maxFrames - startTransformation));
  ...
  updateFrame() {
    ...
    this.queue = this.queue.map((process, i) => {
      ...
      const { humanLike, typewriter, preserveSpaces } = this.props;
      ...
      if ((humanLike || typewriter) && this.frame > startTransformation) {
        ...
        character =
          preserveSpaces && (oldCharacter === " " || newCharacter === " ")
            ? " "
